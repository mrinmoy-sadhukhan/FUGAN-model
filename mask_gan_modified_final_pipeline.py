...
def fit(train_ds, original_ds,test_ds, epoch):
  ...
  for num in range(epoch):
    ...
    for step, (mask_input, real_input, target) in enumerate(dataSet):
      ...
      if (step + 1) % 10 == 0:
          face_generator.save_weights("t_training_checkpoints3/generator/face_gen_epoch{epoch}_step{step}.h5".format(epoch=num+1, step=step+1))
          discriminator.save("t_training_checkpoints3/discriminator/face_dis_epoch{epoch}_step{step}.h5".format(epoch=num+1, step=step+1))
